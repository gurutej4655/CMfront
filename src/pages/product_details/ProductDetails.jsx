...
function ProductDetails() {
  ...
  const addProductToCart = async () => {
    let sellerData = await getAPI(`seller/${productData.sellerId}`);
            ...
            {productData.minimumOrderQuantity <= productData.quantity ? (
              <button
                className={`flex mb-2 md:mb-4 mt-4 md:mt-2  text-white ${
                  userData && isProductInCart
                    ? "bg-amber-500 hover:bg-amber-600"
                    : " bg-[#e11d48] hover:bg-[#e5345a]"
                } border-0 py-4 px-12 focus:outline-none rounded`}
                onClick={(e) => {
                  e.preventDefault();
                  if (userData && isProductInCart) {
                    removeProductFromCart();
                  } else {
                    if (userData) addProductToCart();
                    else navigate("/account/user");
                  }
                }}
              >
                {userData && isProductInCart ? (
